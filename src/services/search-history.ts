...
// Types for search history items
...
class SearchHistoryService {
   ...
   */
  addAirportToHistory(airport: Omit<AirportSearchItem, 'timestamp'>): void {
    try {
      const history = this.getAirportHistory();
      
      // Create new history item with timestamp
      const newItem: AirportSearchItem = {
        ...airport,
        timestamp: Date.now()
      };
      ...
      const filteredHistory = history.filter(
        item => !(item.code === airport.code && item.type === airport.type)
      );
   ...
   */
  addFlightSearchToHistory(search: Omit<FlightSearchItem, 'timestamp'>): void {
    try {
      const history = this.getFlightHistory();
      
      // Create new history item with timestamp
      const newItem: FlightSearchItem = {
        ...search,
        timestamp: Date.now()
      };
   ...
   */
  getAirportHistory(): AirportSearchItem[] {
    try {
   ...
   */
  getFlightHistory(): FlightSearchItem[] {
    try {
   ...
   */
  getAirportHistoryByType(type: 'origin' | 'destination'): AirportSearchItem[] {
    const history = this.getAirportHistory();
