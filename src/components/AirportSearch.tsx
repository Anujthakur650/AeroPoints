...
function LegacyAirportSearch(props: AirportSearchProps) {
  ...
  const handleSearch = (searchQuery: string) => {
    ...
    const matchedAirports = popularDestinations.filter(airport => 
  ...
  return (
    <div className="relative">
      ...
      {results.length > 0 && (
        <ul className="absolute z-10 w-full mt-1 bg-gray-800 border border-gray-700 rounded-lg max-h-60 overflow-auto shadow-xl">
          {results.map((airport) => (
            <li
              key={airport.code}
              className="flex justify-between p-3 hover:bg-gray-700 cursor-pointer"
...
function NewAirportSearch({ value, onChange, label, placeholder }: AirportSearchProps) {
  const [query, setQuery] = useState('');
