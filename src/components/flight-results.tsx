...
export function FlightResults({ flights, isLoading, searchParams, onFlightSelect }: FlightResultsProps) {
  const [sortBy, setSortBy] = useState<'points' | 'duration' | 'departure'>('points');
    ...
    const sorted = [...flights].sort((a, b) => {
      switch (sortBy) {
        ...
        case 'duration':
          ...
          const getDurationMinutes = (duration: string | null | undefined) => {
            if (!duration) return 999999; // Put flights without duration at the end
        ...
        case 'departure':
          ...
          if (!a.departureTime) return 1; // Put flights without departure time at the end
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-center">
                      ...
                      <div className="lg:col-span-3">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm text-gray-300">{flight.cabinClass}</span>
                        </div>
                      </div>
                        ...
                        <div className="flex items-center justify-between">
                          <div className="text-center">
                            <div className={`text-xl font-bold ${
                              flight.departureTime ? 'text-white' : 'text-gray-500'
                            }`}>
                              {formatTime(flight.departureTime)}
                            </div>
                            <div className="text-sm text-gray-400">{flight.origin}</div>
                          </div>
                          ...
                          <div className="flex-1 mx-4">
                              ...
                              <div className="relative flex justify-center">
                                <div className="glass-card px-3 py-1 rounded-full">
                                  <span className={`text-xs ${
                                    flight.duration ? 'text-gray-300' : 'text-gray-500'
                                  }`}>
                            ...
                            <div className="text-center mt-2">
                              {flight.layovers && flight.layovers.length > 0 ? (
                                <span className="text-xs text-yellow-400">{flight.layovers.length} stop{flight.layovers.length > 1 ? 's' : ''}</span>
                              ) : flight.departureTime && flight.arrivalTime ? (
                                <span className="text-xs text-green-400">Non-stop</span>
                              ) : (
                                <span className="text-xs text-gray-500">Route details TBD</span>
                              )}
                            </div>
                          </div>
                          ...
                          <div className="text-center">
                            <div className={`text-xl font-bold ${
                              flight.arrivalTime ? 'text-white' : 'text-gray-500'
                            }`}>
                              {formatTime(flight.arrivalTime)}
                            </div>
                            <div className="text-sm text-gray-400">{flight.destination}</div>
                          </div>
                        </div>
                        ...
                        {(!flight.departureTime || !flight.arrivalTime || !flight.duration) && (
                          <div className="mt-2 text-center">
                            <div className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-blue-500/10 border border-blue-500/20">
                              <Icon icon="lucide:info" className="text-xs text-blue-400" />
                              <span className="text-xs text-blue-400">
                                Schedule details available after booking
                              </span>
                            </div>
                          </div>
                        )}
