...
class AirportDataManager {
  ...
  private searchIndex: Fuse<Airport> | null = null;
   ...
   */
  private async fetchAirportData() {
   ...
   */
  private populateFallbackAirports() {
   ...
   */
  public search(query: string, limit: number = 10): AirportSearchResult[] {
   ...
   */
  private basicSearch(query: string, limit: number): AirportSearchResult[] {
    ...
    if (results.length < limit) {
      for (const airport of this.airports.values()) {
        ...
        if (results.some(r => r.code === airport.iata_code)) continue;
   ...
   */
  public calculateDistance(from: string, to: string): number | null {
    ...
    return this.haversineDistance(
      airport1.coordinates[0],
      airport1.coordinates[1],
      airport2.coordinates[0],
      airport2.coordinates[1]
    );
  }
